...
function n_pcs = cpv(lambda_sorted, cpv_threshold)
    % CPV Numero minimo di componenti che superano la soglia CPV
    %   n_pcs = cpv(lambda_sorted, cpv_threshold)
    %   lambda_sorted : vettore degli autovalori (preferibilmente ordinato decrescente)
    %   cpv_threshold : soglia cumulativa (es. 0.95 per il 95%)
    
    % Validazione input
    if isempty(lambda_sorted) || any(lambda_sorted < 0)
        error('lambda_sorted deve essere un vettore non vuoto di autovalori non negativi.');
    end
    if cpv_threshold <= 0 || cpv_threshold > 1
        error('cpv_threshold deve essere in (0,1].');
    end
    
    total_variance = sum(lambda_sorted);
    if total_variance == 0
        warning('Varianza totale nulla: restituisco 1.');
        n_pcs = 1;
        return
    end
    ...
    if isempty(n_pcs)
        n_pcs = numel(lambda_sorted); % se soglia non raggiunta, prendo tutte le componenti
    end
    
    fprintf('Selezionate %d componenti principali (spiegano %.2f%% della varianza).\n', ...
        n_pcs, cum_variance(n_pcs)*100);
end


function [n_pcs_opt, VRE] = select_npcs_vre(R, P_all, lambda_sorted)
%--------------------------------------------------------------------------
% VRE-based selection of number of principal components
% Based on Li et al. (2000), Eq. (37) and Eq. (38)
%
% INPUT:
%   R              : correlation (or covariance) matrix (m x m)
%   P_all          : eigenvectors of R (columns sorted by eigenvalues)
%   lambda_sorted  : eigenvalues of R (descending order)
%
% OUTPUT:
%   n_pcs_opt      : optimal number of PCs (minimum VRE)
%   VRE            : VRE value for each l = 1,...,m-1
%--------------------------------------------------------------------------

m = size(R,1);
VRE = zeros(m-1,1);

% Variance of each variable (for correlation matrix this is 1)
var_x = diag(R);
...
for l = 1:(m-1)
    ...
    u = zeros(m,1);   % reconstruction error variance for each variable
    ...
    for i = 1:m
        ...
        % Eq. (37) - Variance of reconstruction error
        numerator = r_ii ...
                    - 2 * (c_i' * r_i) ...
                    + (c_i' * R * c_i);
        
        denominator = (1 - c_ii)^2;
...
% Optimal number of PCs = minimum VRE
[~, n_pcs_opt] = min(VRE);
...
[n_pcs_vre, VRE_values] = select_npcs_vre(R, P_all, lambda_sorted);
fprintf('Selezione VRE: %d componenti principali (VRE minima = %.4f)\n', ...
    n_pcs_vre, VRE_values(n_pcs_vre));

% Usa VRE invece di CPV
n_pcs = n_pcs_vre;
P = P_all(:, 1:n_pcs);
...
for k = (n_init + 1) : n_obs
    ...
    if ~is_fault
        ...
        cpv(lambda_sorted,cpv_threshold);
        P = P_all(:, 1:n_pcs);
